#include "utils.h"

static const uint8_t wordWidth = sizeof(size_t)*8;

uint16_t set_bits(size_t * payload, uint8_t offset, uint8_t range, uint16_t value){
    if (value >= 1 << range){ //ensure value can fit in desired range
        //printf("failed to set. Invalid value\n");
        value &= ((1 << range)-1); // mask any bits off of our value in case the user value is out of range
    }
    uint16_t word = (offset / wordWidth);  //find the word offset from bit offset
    uint16_t shift = offset - word*wordWidth;  //find shift amount required from start of word
    size_t * address = payload+word;  //get address of the word
    size_t mask = ((1 << range)-1)<<shift;  //create a mask the size of our value
    *address &= ~mask;  //clear only the bits that will hold our value
    *address |= (size_t)value << (shift);  //shift our value into position and OR them into the word.
    if (shift + range >= wordWidth){  //if the value wraps around to the next word, run the algo again.
        address += 1;  //get the next word
        size_t newValue = value >> (wordWidth - shift);  //shift out the bits we have already written
        set_bits(payload, wordWidth*(word+1), (range - (wordWidth - shift)), newValue);  //run the algo again with ramaining piece of value and new offset
    }
    return 0;
}

uint16_t get_bits(size_t * payload, uint8_t offset, uint8_t range){
    uint16_t word = (offset / wordWidth);  //find word offset from bit offset
    uint16_t shift = offset - word*wordWidth;  //find shift amount required from start of word
    size_t * address = payload+word;  //get address of the word
    size_t val = *address >> shift;  //shift the bits, get desired value
    if (shift + range > wordWidth){  //if the value wraps around to the next word, run the algo again.
        address += 1;  //get the next word
        uint16_t newRange = range-(wordWidth-shift);  //calculate how many bits are left to get
        val += get_bits(payload, wordWidth*(word+1), newRange) << (wordWidth-shift);  //run algo again, and shift the next set of bits into our result
    } else{  //the value is in the LSB position but we need to mask everything above it.
        val &= (1 << range)-1;  //create a mask the same length as our value and mask the result
    }
    return val;
}


static const uint8_t CRC_TABLE[256] = {
    
};

uint8_t crc8ccitt(uint8_t * data, uint8_t size) {
	uint8_t val = 0;

	uint8_t * pos = (uint8_t *) data;
	uint8_t * end = pos + size;

	while (pos < end) {
		val = CRC_TABLE[val ^ *pos];
		pos++;
	}

	return val;
}