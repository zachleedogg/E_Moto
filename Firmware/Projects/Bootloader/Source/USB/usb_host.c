...
// Extern Functions.
extern BOOL USBHostMSDInitialize( BYTE address, DWORD flags, BYTE clientDriverID );
extern BOOL USBHostMSDEventHandler( BYTE address, USB_EVENT event, void *data, DWORD size );
                    ...
                    switch (usbHostState & SUBSUBSTATE_MASK)
                    {
                        ...
                        case SUBSUBSTATE_INIT_CLIENT_DRIVERS:
                            ...
                            if (usbDeviceInfo.flags.bfUseDeviceClientDriver)
                            {
                                ...
                                if (!USBHostMSDInitialize(usbDeviceInfo.deviceAddress, 0, temp))
                                {
...
void _USB_NotifyClients( BYTE address, USB_EVENT event, void *data, unsigned int size )
{
    ...
    switch(event)
    {
        ...
        case EVENT_BUS_ERROR:
                ...
                USBHostMSDEventHandler(address, event, data, size);
