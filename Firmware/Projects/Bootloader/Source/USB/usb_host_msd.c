...
//******************************************************************************
  ...
  #define _USBHostMSD_TerminateTransfer( error )    {                                                                           \
    ...
    #define _USBHostMSD_TerminateTransfer( error )  {                                                                                                           \
                                                        ...
                                                        USBHostMSDSCSIEventHandler( deviceInfoMSD[i].deviceAddress, EVENT_MSD_TRANSFER, NULL, 0 );     \
                                                    }
  #else
    #define _USBHostMSD_TerminateTransfer( error )  {                                                                                                           \
                                                        deviceInfoMSD[i].errorCode  = error;                                                                    \
...
//******************************************************************************
//******************************************************************************
// Section: MSD Host External Functions
//******************************************************************************
//******************************************************************************
                ...
                case STATE_INITIALIZE_DEVICE:
                    switch (deviceInfoMSD[i].state & SUBSTATE_MASK)
                    {
                        ...
                        case SUBSTATE_GET_MAX_LUN_COMPLETE:
                            ...
                            if (USBHostMSDSCSIInitialize( deviceInfoMSD[i].deviceAddress, 0, 0 ))
                            {
                                USBHostMSDSCSIEventHandler( deviceInfoMSD[i].deviceAddress, EVENT_MSD_MAX_LUN, &(deviceInfoMSD[i].maxLUN), 1 );
    ...
    switch (event)
    {
        ...
        case EVENT_DETACH:           // USB cable has been detached (data: BYTE, address of device)
            ...
            if (i < USB_MAX_MASS_STORAGE_DEVICES)
            {
                ...
                USBHostMSDSCSIEventHandler( address, EVENT_DETACH, NULL, 0 );
            }
            return TRUE;
            break;
        ...
        case EVENT_TRANSFER:         // A USB transfer has completed - optional
                ...
                switch (deviceInfoMSD[i].state)
                {
                    case STATE_WAIT_FOR_MAX_LUN:
                        ...
                        // Tell the media interface layer that we have a MSD attached.
                        if (USBHostMSDSCSIInitialize( deviceInfoMSD[i].deviceAddress, usbMediaInterfaceTable.flags, 0 ))
                        {
                            USBHostMSDSCSIEventHandler( deviceInfoMSD[i].deviceAddress, EVENT_MSD_MAX_LUN, &(deviceInfoMSD[i].maxLUN), 1 );
                            deviceInfoMSD[i].state = STATE_RUNNING;
                        }
...
void _USBHostMSD_ResetStateJump( BYTE i )
{
    ...
    {
        USBHostMSDSCSIEventHandler( deviceInfoMSD[i].deviceAddress, EVENT_MSD_RESET, NULL, 0 );
